...
if not vim.env._ZO_DATA_DIR then
  ...
  if vim.fn.has 'mac' == 1 then
    data_dir = vim.fn.expand '$HOME/Library/Application Support'
  elseif vim.fn.has 'win32' == 1 then
    data_dir = vim.fn.expand '$LOCALAPPDATA'
  else
    -- Linux/BSD - use XDG_DATA_HOME or default to ~/.local/share
    data_dir = vim.env.XDG_DATA_HOME or vim.fn.expand '$HOME/.local/share'
  end
